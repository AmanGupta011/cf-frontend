// status after we enter submission
  ...
  return (
    <div>
      <div className="max-w-[1200px] mx-8 xl:mx-auto">
        ...
        <div className="mt-4">
          {queue && (
            <h1 className="text-red-500 font-bold font text-2xl">{`Submission is in queue...`}</h1>
          )}
          {
            <h1 className="text-red-500 font-bold font text-2xl">
              {errorMessage}
            </h1>
          }
        </div>
        <div className="mt-4">
          {ticket.progress === "processed" && ticket.verdict == false && (
            <h1 className="text-red-500 font-bold font text-2xl">
              {ticket.error}
            </h1>
          )}
      ...
      {ticket.progress === "processed" && ticket.verdict === true && (
        <div className="max-w-[1200px] mx-8 xl:mx-auto flex flex-col justify-center mt-3">
          <div className="w-full text-xl bg-zinc-300 mt-8 py-2 flex flex-row justify-between items-center px-4">
            ...
            <div>
              <span
                ...
                onClick={(e) => navigator.clipboard.writeText(inputText)}
              >
                <FaRegCopy />
              </span>
            </div>
          </div>
          <pre className="bg-zinc-100 px-4 py-3">{inputText}</pre>
          <div className="w-full text-xl bg-zinc-300 mt-8 py-2 flex flex-row justify-between items-center px-4">
            <div className="font-semibold">YOUR OUTPUT</div>
            <div>
              <span
                className="cursor-pointer"
                onClick={(e) => navigator.clipboard.writeText(userOutputText)}
              >
                <FaRegCopy />
              </span>
            </div>
          </div>
          <pre className="bg-zinc-100 px-4 py-3">{userOutputText}</pre>
          <div className="w-full text-xl bg-zinc-300 mt-8 py-2 flex flex-row justify-between items-center px-4">
            ...
            <div>
              <span
                className="cursor-pointer"
                onClick={(e) =>
                  navigator.clipboard.writeText(expectedOutputText)
                }
              >
                <FaRegCopy />
              </span>
          ...
          <pre className="bg-zinc-100 px-4 py-3 mb-8">{expectedOutputText}</pre>
        </div>
      )}
    </div>
  );
